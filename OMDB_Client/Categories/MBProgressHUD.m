...
- (void)done;
- (void)updateLabelText:(NSString *)theNewText;
- (void)updateDetailsLabelText:(NSString *)theNewText;
- (void)updateProgress;
...
- (void)setLabelText:(NSString *)theNewText {
	if ([NSThread isMainThread]) {
		[self updateLabelText:theNewText];
		[self setNeedsLayout];
		[self setNeedsDisplay];
	} else {
		[self performSelectorOnMainThread:@selector(updateLabelText:) withObject:theNewText waitUntilDone:NO];
		[self performSelectorOnMainThread:@selector(setNeedsLayout) withObject:nil waitUntilDone:NO];
		[self performSelectorOnMainThread:@selector(setNeedsDisplay) withObject:nil waitUntilDone:NO];
	}
}
...
- (void)setDetailsLabelText:(NSString *)theNewText {
	if ([NSThread isMainThread]) {
		[self updateDetailsLabelText:theNewText];
		[self setNeedsLayout];
		[self setNeedsDisplay];
	} else {
		[self performSelectorOnMainThread:@selector(updateDetailsLabelText:) withObject:theNewText waitUntilDone:NO];
		[self performSelectorOnMainThread:@selector(setNeedsLayout) withObject:nil waitUntilDone:NO];
		[self performSelectorOnMainThread:@selector(setNeedsDisplay) withObject:nil waitUntilDone:NO];
	}
}
...
- (void)setProgress:(float)theNewProgress {
    progress = theNewProgress;
...
- (void)updateLabelText:(NSString *)theNewText {
    if (labelText != theNewText) {
        labelText = [theNewText copy];
    }
}

- (void)updateDetailsLabelText:(NSString *)theNewText {
    if (detailsLabelText != theNewText) {
        detailsLabelText = [theNewText copy];
    }
}
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        if (nil != self.detailsLabelText) {
            ...
            CGSize labelSize = [detailsLabel.text sizeWithAttributes:@{NSFontAttributeName:detailsLabel.font}];
